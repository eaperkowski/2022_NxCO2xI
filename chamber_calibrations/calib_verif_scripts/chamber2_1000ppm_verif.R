...
#
#   - Day humidity offset:    -8.0        Night humidity offset:  -13.0
#   - Day auxillary offset:  102.0        Night auxillary offset:  88.4
...
###############################################################################
licor <- licorData("../calib_verif_files/c2_1000ppm_calib_verif_licor") %>%
  ...
  filter(date > "2022-06-06 11:05:00")

chamber2 <- read.csv("../calib_verif_files/c2_1000ppm_calib_verif.csv") %>%
...
dayco2.summary <- li.dayco2.summary %>%
  ...
  mutate(co2.offset = (co2.mean[2] - co2.mean[1]),
         co2.offset.actual = 102 - co2.offset) %>%
  data.frame()
dayco2.summary
...
# Chamber 1 night CO2 offset: 125.7 ppm CO2
...
ch2.25C <- subset(chamber2, date > "2022-06-06 11:45:00" & date < "2022-06-06 12:30:00")
...
# Chamber 1 25degC offset: 4.0 deg C; although current offsets are within ci
# range
...
###############################################################################
# Subset Licor and chamber measurements that were set at 25degC
li.17C <- subset(licor, date > "2022-06-06 13:15:00" & date < "2022-06-06 14:00:00")
ch2.17C <- subset(chamber2, date > "2022-06-06 13:15:00" & date < "2022-06-06 14:00:00")
