...
#   - Day temp point 2:       25.0        Day temp offset 2:         4.5
#   - Day temp point 3:       45.0        Day temp offset 3:         0.0
#
#   - Night temp point 1:     17.0        Night temp offset 1:       6.3
#   - Night temp point 2:     35.0        Night temp offset 2:       0.0 
#   - Night temp point 3:     45.0        Night temp offset 3:       0.0
#
#   - Day humidity offset:   -11.0        Night humidity offset:   -15.2
#   - Day auxillary offset:   90.7        Night auxillary offset:   59.0
...
###############################################################################
licor <- licorData("../calib_verif_files/c5_1000ppm_calib_verif_licor2") %>%
...
chamber5 <- read.csv("../calib_verif_files/c5_1000ppm_calib_verif2.csv") %>%
...
dayco2.summary <- li.dayco2.summary %>%
  ...
  mutate(co2.offset = (co2.mean[2] - co2.mean[1]),
         co2.offset.actual = 90.7 - co2.offset) %>%
  data.frame()
dayco2.summary

# Chamber 5 day CO2 offset: 115.5 ppm CO2
...
nightco2.summary <- li.nightco2.summary %>%
  ...
  mutate(co2.offset = (co2.mean[2] - co2.mean[1]),
         co2.offset.actual = 59 - co2.offset) %>%
  data.frame()
nightco2.summary

# Chamber 5 night CO2 offset: 69.4 ppm CO2
...
ch5.25C <- subset(chamber5, date > "2022-06-09 14:00:00")
...
temp.25C.summary <- li.25C.summary %>%
  ...
  mutate(temp.offset = temp.mean[2] - temp.mean[1],
         temp.offset.actual = 4.5 - temp.offset) %>%
  data.frame()
temp.25C.summary
...
###############################################################################
# Subset Licor and chamber measurements that were set at 25degC
li.17C <- subset(licor, date > "2022-06-09 12:00:00" & date < "2022-06-09 13:00:00")
ch5.17C <- subset(chamber5, date > "2022-06-09 12:00:00" & date < "2022-06-09 13:00:00")
...
temp.17C.summary <- li.17C.summary %>%
  ...
  mutate(temp.offset = temp.mean[2] - temp.mean[1],
         temp.offset.actual = 6.3 - temp.offset) %>%
  data.frame()
temp.17C.summary
