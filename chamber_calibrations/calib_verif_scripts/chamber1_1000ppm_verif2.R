###############################################################################
## Verification script for eCO2 experiment: chamber 1 @ 1000 ppm CO2
###############################################################################
# Chamber program:
#
# Constants:
...
#
# Program offsets from initial script
...
#   - Day auxillary offset:  101.0        Night auxillary offset:  88.0
...
###############################################################################
licor <- licorData("../calib_verif_files/c1_1000ppm_calib_verif_licor2") %>%
  ...
  filter(date > "2022-06-10 14:00" & date < "2022-06-10 17:45")

chamber1 <- read.csv("../calib_verif_files/c1_1000ppm_calib_verif2.csv") %>%
...
# Temperature
temp <- ggplot() +
  geom_line(data = chamber1, aes(x = as.POSIXct(date), 
                                 y = measured_temp, 
                                 color = "measured"),
            size = 0.5) +
  geom_line(data = chamber1, aes(x = as.POSIXct(date), 
  ...
  scale_color_manual(values = c("red", "blue", "black"),
                     labels = c("Licor",
                                "Chamber sensor", 
                                "Chamber set point")) +
  scale_x_datetime(limits = c(as.POSIXct("2022-06-10 14:00"),
                              as.POSIXct("2022-06-10 17:45")),
                   breaks = c(as.POSIXct("2022-06-10 14:00"),
                              as.POSIXct("2022-06-10 14:45"),
                              as.POSIXct("2022-06-10 15:30"),
                              as.POSIXct("2022-06-10 16:15"),
                              as.POSIXct("2022-06-10 17:00"),
                              as.POSIXct("2022-06-10 17:45")),
                   date_labels = "%R") +
...
day.ch1 <- subset(chamber1,  prog_temp > 17)

day.co2 <- ggplot() +
  geom_density(data = day.ch1, 
...
dayco2.summary <- li.dayco2.summary %>%
  full_join(ch1.dayco2.summary) %>%
  mutate(co2.offset = (co2.mean[2] - co2.mean[1]),
         co2.offset.actual = 101 - co2.offset) %>%
  data.frame()
dayco2.summary
...
night.ch1 <- subset(chamber1, prog_temp == 17)

night.co2 <- ggplot() +
  geom_density(data = night.ch1, 
               aes(x = as.numeric(measured_co2), fill = "chamber"), alpha = 0.75) +
...
# Calculate mean, ci, uci, and lci of chamber sensor measurements at 25degC
ch1.nightco2.summary <- night.ch1 %>%
  summarize(co2.mean = mean(as.numeric(measured_co2), na.rm = TRUE),
            co2.ci = 1.96 + (sd(measured_co2)/sqrt(length(measured_co2))),
            co2.uci = co2.mean + co2.ci,
            co2.lci = co2.mean - co2.ci) %>%
...
nightrh.summary <- li.nightrh.summary %>%
  full_join(ch1.nightrh.summary) %>%
  mutate(rh.offset = (rh.mean[2] - rh.mean[1]),
         rh.offset.actual = -12.9 - rh.offset) %>%
  data.frame()
nightrh.summary
...
li.25C <- subset(licor, date > "2022-06-06 11:45:00" & date < "2022-06-06 12:30:00")
ch1.25C <- subset(chamber1, date > "2022-06-06 11:45:00" & date < "2022-06-06 12:30:00")

# Visualize density plots of chamber sensor temperature and licor Tair
dens.25C <- ggplot() +
  geom_density(data = ch1.25C, 
  ...
  scale_x_continuous(limits = c(22.5, 25.5), breaks = seq(22.5,25.5,1)) +
  scale_fill_brewer(palette = "Spectral", labels = c("Chamber w/o offset",
                                                     "Licor")) +
...
###############################################################################
# Subset Licor and chamber measurements that were set at 25degC
li.17C <- subset(licor, date > "2022-06-06 13:45:00" & date < "2022-06-06 14:30:00")
ch1.17C <- subset(chamber1, date > "2022-06-06 13:45:00" & date < "2022-06-06 14:30:00")

# Visualize density plots of chamber sensor temperature and licor Tair
dens.17C <- ggplot() +
  geom_density(data = ch1.17C, 
               aes(x = as.numeric(measured_temp), 
                   fill = "chamber"), alpha = 0.75) +
